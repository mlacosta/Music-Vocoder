...
void envelopBufferingtTest(float Coefficientes [CANTDEFILTROS][6][CANTDEETAPAS],float BufferAudio [],float BufferSynth [],float SenalProcesada [],
             ...
             float EstadoDelay [CANTDEFILTROS][ORDENDELAY+1],int CantDeBuffers,float CoefLPF[][ETAPASLPF],float EstadoLPF [][ETAPASLPF],float gainLPF ){

    FILE* BPF;
    FILE* bf ;
    FILE* Resultado = fopen("resultadoEnvolventeMultiple.dat","w");
    ...
    for(int y = 0; y<CantDeBuffers; y++){
        ...
        if (Resultado == NULL){
            printf("Open resultado failed (%s). \n\n---------------------------\n\n","resultadoEnvolventeMultiple.dat");
            return;
        }else{
            printf("\nOpen resultado OK (%s). \n\n\n","resultadoEnvolventeMultiple.dat");
            envolvente (CoefHilbert, BufferAudio,SenalProcesada,EstadoHilbert[3],EstadoDelay[3],CoefLPF,EstadoLPF,gainLPF );
            printf("Vocoder Done. \n\n---------------------------\n\n");
